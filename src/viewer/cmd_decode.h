...
// record for the command log and (b), via shadow_state.h, a maintained shadow of
// RDP state for the inspector. One decoder feeds both panels — no rendering, SDL
// or ImGui dependency.
//
// Command IDs are the 6-bit field (word0 >> 24) & 0x3f, matching CMD_ID() in
// src/rdp/n64video.c. Word counts come from the renderer's rdp_commands[] length
// table (bytes/4). Unknown / unhandled opcodes fall back to a raw-hex record and
// a 1-word-pair (2-word) length guess — the decoder NEVER crashes on bad input.
...
// src/rdp/n64video.c CMD_ID_*). Listed here so the decoder/test do not depend on
// rdp internal headers.
enum {
  RDPCMD_NO_OP = 0x00,
...
// A decoded record for one command. The decoder fills `name`, `id`, `word_count`,
// `known`, and a one-line `summary` of the decoded fields. The log panel prints
// `summary`; the inspector reads the shadow (shadow_state.h) instead.
//
// `summary` is a fixed buffer (no heap, Orthodox); long fields are formatted to
// fit. `known` is 0 for unhandled/invalid opcodes (raw-hex fallback in summary).
struct CmdRecord {
...
// Decode the command at `words` (which has at least `avail` words available) into
// `out`. Reads at most rdp_cmd_word_count(id) words but never more than `avail`
// (a truncated tail is reported in the summary rather than over-read). Returns
// the word count the decoder *would* consume (>= 2), so a caller can iterate a
// packed stream: advance by min(returned, remaining). Never crashes.
uint32_t cmd_decode(const uint32_t *words, uint32_t avail, struct CmdRecord *out);
