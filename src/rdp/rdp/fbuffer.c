...
static void fbwrite_4(uint32_t wid, uint32_t curpixel, uint32_t /*r*/,
  ...
  uint32_t const fb = state[wid].fb_address + curpixel;
  RWRITEADDR8(fb, 0);
}
...
#undef CVG_DRAW
  ...
  int32_t finalcvg = finalize_spanalpha(
      state[wid].other_modes.cvg_dest, blend_en, curpixel_cvg, curpixel_memcvg);
  int16_t finalcolor;
...
static void fbwrite_32(uint32_t wid, uint32_t curpixel, uint32_t r, uint32_t g,
                       uint32_t b, uint32_t blend_en, uint32_t curpixel_cvg,
                       uint32_t curpixel_memcvg) {
  uint32_t const fb = (state[wid].fb_address >> 2) + curpixel;

  int32_t finalcolor;
  int32_t const finalcvg = finalize_spanalpha(
      state[wid].other_modes.cvg_dest, blend_en, curpixel_cvg, curpixel_memcvg);
...
static void fbfill_8(uint32_t wid, uint32_t curpixel) {
  uint32_t const fb = state[wid].fb_address + curpixel;
  uint32_t const val = (state[wid].fill_color >> (((fb & 3) ^ 3) << 3)) & 0xff;
...
static void fbfill_16(uint32_t wid, uint32_t curpixel) {
  uint16_t val;
  uint8_t hval;
  uint32_t const fb = (state[wid].fb_address >> 1) + curpixel;
  if (fb & 1) {
...
static void fbfill_32(uint32_t wid, uint32_t curpixel) {
  uint32_t const fb = (state[wid].fb_address >> 2) + curpixel;
  PAIRWRITE32(fb, state[wid].fill_color,
              (state[wid].fill_color & 0x10000) ? 3 : 0,
              (state[wid].fill_color & 0x1) ? 3 : 0);
}

static void fbread_4(uint32_t wid, uint32_t /*curpixel*/,
                     uint32_t* curpixel_memcvg) {
  state[wid].memory_color.r = state[wid].memory_color.g =
      state[wid].memory_color.b = 0;
...
static void fbread2_4(uint32_t wid, uint32_t /*curpixel*/,
                      uint32_t* curpixel_memcvg) {
  state[wid].pre_memory_color.r = state[wid].pre_memory_color.g =
      state[wid].pre_memory_color.b = 0;
...
static void fbread_8(uint32_t wid, uint32_t curpixel,
                     uint32_t* curpixel_memcvg) {
  uint8_t mem;
  uint32_t const addr = state[wid].fb_address + curpixel;
  RREADADDR8(mem, addr);
  state[wid].memory_color.r = state[wid].memory_color.g =
      state[wid].memory_color.b = mem;
  *curpixel_memcvg = 7;
  state[wid].memory_color.a = 0xe0;
}
...
static void fbread2_8(uint32_t wid, uint32_t curpixel,
  ...
  RREADADDR8(mem, addr);
  state[wid].pre_memory_color.r = state[wid].pre_memory_color.g =
      state[wid].pre_memory_color.b = mem;
  state[wid].pre_memory_color.a = 0xe0;
  *curpixel_memcvg = 7;
}
...
static void fbread_16(uint32_t wid, uint32_t curpixel,
  ...
  uint16_t fword;
  uint8_t hbyte;
  uint32_t const addr = (state[wid].fb_address >> 1) + curpixel;
  ...
  if (state[wid].other_modes.image_read_en) {
    ...
    if (state[wid].fb_format == FORMAT_RGBA) {
      ...
      state[wid].memory_color.r = state[wid].memory_color.g =
          state[wid].memory_color.b = fword >> 8;
      lowbits = (fword >> 5) & 7;
    }
...
static void fbread2_16(uint32_t wid, uint32_t curpixel,
  ...
  if (state[wid].other_modes.image_read_en) {
    ...
    if (state[wid].fb_format == FORMAT_RGBA) {
      ...
      state[wid].pre_memory_color.r = state[wid].pre_memory_color.g =
          state[wid].pre_memory_color.b = fword >> 8;
      lowbits = (fword >> 5) & 7;
    }
...
static void fbread_32(uint32_t wid, uint32_t curpixel,
                      uint32_t* curpixel_memcvg) {
  uint32_t mem;
  uint32_t const addr = (state[wid].fb_address >> 2) + curpixel;
  RREADIDX32(mem, addr);
