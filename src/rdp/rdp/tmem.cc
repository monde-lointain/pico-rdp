...
static void sort_tmem_idx(uint32_t *idx, uint32_t idxa, uint32_t idxb,
                          uint32_t idxc, uint32_t idxd, uint32_t bankno) {
  if ((idxa & 3) == bankno) {
    *idx = idxa & 0x3ff;
...
static void sort_tmem_shorts_lowhalf(uint32_t *bindshort, uint32_t short0,
                                     uint32_t short1, uint32_t short2,
...
static void compute_color_index(uint32_t wid, uint32_t *cidx,
                                uint32_t readshort, uint32_t nybbleoffset,
                                uint32_t tilenum) {
  uint32_t lownib;
  uint32_t hinib;
...
void fetch_texel(uint32_t wid, struct Color *color, int s, int t,
  ...
  switch (rdpxi_state[wid].tile[tilenum].f.notlutswitch) {
    ...
    case TEXEL_YUV16: {
      ...
      uint16_t const c = TC16[taddrlow];
...
void fetch_texel_quadro(uint32_t wid,  // NOLINT(readability-function-size)
                        struct Color *color0, struct Color *color1,
                        struct Color *color2, struct Color *color3, int s0,
                        int sdiff, int t0, int tdiff, uint32_t tilenum,
                        int unequaluppers) {
  ...
  switch (rdpxi_state[wid].tile[tilenum].f.notlutswitch) {
    ...
    case TEXEL_CI4: {
      ...
      p = (ands) ? (p & 0xf) : (p >> 4);
      p = (tpal << 4) | p;
      color0->r = color0->g = color0->b = color0->a = (int32_t)p;
      p = rdpxi_state[wid].tmem[taddr2];
      p = (ands) ? (p & 0xf) : (p >> 4);
      p = (tpal << 4) | p;
      color2->r = color2->g = color2->b = color2->a = (int32_t)p;

      ands = s1 & 1;
      p = rdpxi_state[wid].tmem[taddr1];
      p = (ands) ? (p & 0xf) : (p >> 4);
      p = (tpal << 4) | p;
      color1->r = color1->g = color1->b = color1->a = (int32_t)p;
      p = rdpxi_state[wid].tmem[taddr3];
      p = (ands) ? (p & 0xf) : (p >> 4);
      p = (tpal << 4) | p;
      color3->r = color3->g = color3->b = color3->a = (int32_t)p;
    } break;
    case TEXEL_CI8: {
      taddr0 = (tbase0 << 3) + s0;
      taddr1 = (tbase0 << 3) + s1;
    ...
    case TEXEL_IA8: {
      ...
      color0->g = (int32_t)i;
      color0->b = (int32_t)i;
      color0->a = (int32_t)(((p & 0xf) << 4) | (p & 0xf));
      p = rdpxi_state[wid].tmem[taddr1];
      i = p & 0xf0;
      i |= (i >> 4);
      color1->r = (int32_t)i;
      color1->g = (int32_t)i;
      color1->b = (int32_t)i;
      color1->a = (int32_t)(((p & 0xf) << 4) | (p & 0xf));
      p = rdpxi_state[wid].tmem[taddr2];
      i = p & 0xf0;
      i |= (i >> 4);
      color2->r = (int32_t)i;
      color2->g = (int32_t)i;
      color2->b = (int32_t)i;
      color2->a = (int32_t)(((p & 0xf) << 4) | (p & 0xf));
      p = rdpxi_state[wid].tmem[taddr3];
      i = p & 0xf0;
      i |= (i >> 4);
      color3->r = (int32_t)i;
      color3->g = (int32_t)i;
      color3->b = (int32_t)i;
      color3->a = (int32_t)(((p & 0xf) << 4) | (p & 0xf));
    } break;
    case TEXEL_IA16: {
      taddr0 = (tbase0 << 2) + s0;
      taddr1 = (tbase0 << 2) + s1;
    ...
    case TEXEL_I4: {
      ...
      c0 = ands ? (p & 0xf) : (p >> 4);
      c0 |= (c0 << 4);
      color0->r = color0->g = color0->b = color0->a = (int32_t)c0;
      p = rdpxi_state[wid].tmem[taddr2];
      c2 = ands ? (p & 0xf) : (p >> 4);
      c2 |= (c2 << 4);
      color2->r = color2->g = color2->b = color2->a = (int32_t)c2;

      ands = s1 & 1;
      p = rdpxi_state[wid].tmem[taddr1];
      c1 = ands ? (p & 0xf) : (p >> 4);
      c1 |= (c1 << 4);
      color1->r = color1->g = color1->b = color1->a = (int32_t)c1;
      p = rdpxi_state[wid].tmem[taddr3];
      c3 = ands ? (p & 0xf) : (p >> 4);
      c3 |= (c3 << 4);
      color3->r = color3->g = color3->b = color3->a = (int32_t)c3;
...
void get_tmem_idx(uint32_t wid, int s, int t, uint32_t tilenum, uint32_t *idx0,
  ...
  *bit3flipped = ((sshorts & 2) != 0) ^ (t & 1);
  ...
  int tidx_a = (int)(((tbase << 2) + sshorts) & 0x7fd);
...
void read_tmem_copy(uint32_t wid, int s, int s1, int s2, int s3, int t,
  ...
  tbase <<= 4;
  tidx_a = (int)((tbase + shbytes) & 0x1fff);
  tidx_bhi = (int)((tbase + shbytes1) & 0x1fff);
  tidx_c = (int)((tbase + shbytes2) & 0x1fff);
  tidx_dhi = (int)((tbase + shbytes3) & 0x1fff);

  if (tformat == FORMAT_YUV) {
    delta = (int32_t)(shbytes1 - shbytes);
    tidx_blow = (tidx_a + (delta << 1)) & 0x1fff;
    tidx_dlow = (int)((tidx_blow + shbytes3 - shbytes) & 0x1fff);
  } else {
    tidx_blow = tidx_bhi;
    tidx_dlow = tidx_dhi;
  }
  ...
  sort_tmem_shorts_lowhalf(&sortshort[0], short0, short1, short2, short3,
                           lowbits[0] >> 2);
  sort_tmem_shorts_lowhalf(&sortshort[1], short0, short1, short2, short3,
                           lowbits[1] >> 2);
  sort_tmem_shorts_lowhalf(&sortshort[2], short0, short1, short2, short3,
                           lowbits[3] >> 2);
  ...
  if (rdpxi_state[wid].other_modes.en_tlut) {
    ...
    sort_tmem_shorts_lowhalf(&sortshort[4], short0, short1, short2, short3,
                             lowbits[0] >> 2);
    sort_tmem_shorts_lowhalf(&sortshort[5], short0, short1, short2, short3,
                             lowbits[2] >> 2);
    sort_tmem_shorts_lowhalf(&sortshort[6], short0, short1, short2, short3,
                             lowbits[3] >> 2);
    sort_tmem_shorts_lowhalf(&sortshort[7], short0, short1, short2, short3,
                             lowbits[5] >> 2);
  }
}
