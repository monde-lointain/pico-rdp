...
// `state[]` array and the command table are declared static directly. The only
// external symbols this TU exposes stay rdpx_*-prefixed (see n64video.c).
...
static struct RdpState state[RDPX_PARALLEL_MAX_WORKERS];
...
static void deduce_derivatives(uint32_t wid) {
  ...
  state[wid].other_modes.f.partialreject_1cycle =
      (state[wid].blender2b_a[0] == &state[wid].inv_pixel_color.a &&
       state[wid].blender1b_a[0] == &state[wid].pixel_color.a);
  state[wid].other_modes.f.partialreject_2cycle =
      (state[wid].blender2b_a[1] == &state[wid].inv_pixel_color.a &&
       state[wid].blender1b_a[1] == &state[wid].pixel_color.a);
  ...
  state[wid].other_modes.f.realblendershiftersneeded =
      (special_bsel0 && state[wid].other_modes.cycle_type == CYCLE_TYPE_1) ||
      (special_bsel1 && state[wid].other_modes.cycle_type == CYCLE_TYPE_2);
  state[wid].other_modes.f.interpixelblendershiftersneeded =
      (special_bsel0 && state[wid].other_modes.cycle_type == CYCLE_TYPE_2);

  state[wid].other_modes.f.rgb_alpha_dither =
      (state[wid].other_modes.rgb_dither_sel << 2) |
      state[wid].other_modes.alpha_dither_sel;
  ...
  if ((state[wid].combiner_rgbmul_r[1] == &state[wid].lod_frac) ||
      (state[wid].combiner_alphamul[1] == &state[wid].lod_frac)) {
    lod_frac_used_in_cc1 = 1;
  }
  if ((state[wid].combiner_rgbmul_r[0] == &state[wid].lod_frac) ||
      (state[wid].combiner_alphamul[0] == &state[wid].lod_frac)) {
    lod_frac_used_in_cc0 = 1;
  }

  if (state[wid].combiner_rgbmul_r[1] == &state[wid].texel1_color.r ||
      state[wid].combiner_rgbsub_a_r[1] == &state[wid].texel1_color.r ||
      state[wid].combiner_rgbsub_b_r[1] == &state[wid].texel1_color.r ||
      state[wid].combiner_rgbadd_r[1] == &state[wid].texel1_color.r ||
      state[wid].combiner_alphamul[1] == &state[wid].texel1_color.a ||
      state[wid].combiner_alphasub_a[1] == &state[wid].texel1_color.a ||
      state[wid].combiner_alphasub_b[1] == &state[wid].texel1_color.a ||
      state[wid].combiner_alphaadd[1] == &state[wid].texel1_color.a ||
      state[wid].combiner_rgbmul_r[1] == &state[wid].texel1_color.a) {
    texel1_used_in_cc1 = 1;
  }
  if (state[wid].combiner_rgbmul_r[1] == &state[wid].texel0_color.r ||
      state[wid].combiner_rgbsub_a_r[1] == &state[wid].texel0_color.r ||
      state[wid].combiner_rgbsub_b_r[1] == &state[wid].texel0_color.r ||
      state[wid].combiner_rgbadd_r[1] == &state[wid].texel0_color.r ||
      state[wid].combiner_alphamul[1] == &state[wid].texel0_color.a ||
      state[wid].combiner_alphasub_a[1] == &state[wid].texel0_color.a ||
      state[wid].combiner_alphasub_b[1] == &state[wid].texel0_color.a ||
      state[wid].combiner_alphaadd[1] == &state[wid].texel0_color.a ||
      state[wid].combiner_rgbmul_r[1] == &state[wid].texel0_color.a) {
    texel0_used_in_cc1 = 1;
  }
  if (state[wid].combiner_alphamul[0] == &state[wid].texel1_color.a ||
      state[wid].combiner_alphasub_a[0] == &state[wid].texel1_color.a ||
      state[wid].combiner_alphasub_b[0] == &state[wid].texel1_color.a ||
      state[wid].combiner_alphaadd[0] == &state[wid].texel1_color.a) {
    texel1_used_in_ac0 = 1;
  }
  if (state[wid].combiner_alphamul[0] == &state[wid].texel0_color.a ||
      state[wid].combiner_alphasub_a[0] == &state[wid].texel0_color.a ||
      state[wid].combiner_alphasub_b[0] == &state[wid].texel0_color.a ||
      state[wid].combiner_alphaadd[0] == &state[wid].texel0_color.a) {
    texel0_used_in_ac0 = 1;
  }
  if (state[wid].combiner_rgbmul_r[0] == &state[wid].texel1_color.r ||
      state[wid].combiner_rgbsub_a_r[0] == &state[wid].texel1_color.r ||
      state[wid].combiner_rgbsub_b_r[0] == &state[wid].texel1_color.r ||
      state[wid].combiner_rgbadd_r[0] == &state[wid].texel1_color.r ||
      texel1_used_in_ac0 ||
      state[wid].combiner_rgbmul_r[0] == &state[wid].texel1_color.a) {
    texel1_used_in_cc0 = 1;
  }
  if (state[wid].combiner_rgbmul_r[0] == &state[wid].texel0_color.r ||
      state[wid].combiner_rgbsub_a_r[0] == &state[wid].texel0_color.r ||
      state[wid].combiner_rgbsub_b_r[0] == &state[wid].texel0_color.r ||
      state[wid].combiner_rgbadd_r[0] == &state[wid].texel0_color.r ||
      texel0_used_in_ac0 ||
      state[wid].combiner_rgbmul_r[0] == &state[wid].texel0_color.a) {
  ...
  if (texel1_used_in_cc1 ||
      (state[wid].other_modes.alpha_compare_en && texels_or_lf_used_in_ac0)) {
  ...
  if ((state[wid].other_modes.cycle_type == CYCLE_TYPE_2 &&
  ...
  if ((state[wid].other_modes.cycle_type == CYCLE_TYPE_1 &&
       state[wid].combiner_rgbsub_a_r[1] == &state[wid].noise) ||
      (state[wid].other_modes.cycle_type == CYCLE_TYPE_2 &&
       (state[wid].combiner_rgbsub_a_r[0] == &state[wid].noise ||
        state[wid].combiner_rgbsub_a_r[1] == &state[wid].noise)) ||
  ...
  state[wid].other_modes.f.dolod =
      state[wid].other_modes.tex_lod_en || lodfracused;
}
...
static void rdp_set_other_modes(uint32_t wid, const uint32_t* args) {
  ...
  set_blender_input(
      wid, 0, 0, &state[wid].blender1a_r[0], &state[wid].blender1a_g[0],
      &state[wid].blender1a_b[0], &state[wid].blender1b_a[0],
      state[wid].other_modes.blend_m1a_0, state[wid].other_modes.blend_m1b_0);
  set_blender_input(
      wid, 0, 1, &state[wid].blender2a_r[0], &state[wid].blender2a_g[0],
      &state[wid].blender2a_b[0], &state[wid].blender2b_a[0],
      state[wid].other_modes.blend_m2a_0, state[wid].other_modes.blend_m2b_0);
  set_blender_input(
      wid, 1, 0, &state[wid].blender1a_r[1], &state[wid].blender1a_g[1],
      &state[wid].blender1a_b[1], &state[wid].blender1b_a[1],
      state[wid].other_modes.blend_m1a_1, state[wid].other_modes.blend_m1b_1);
  set_blender_input(
      wid, 1, 1, &state[wid].blender2a_r[1], &state[wid].blender2a_g[1],
      &state[wid].blender2a_b[1], &state[wid].blender2b_a[1],
