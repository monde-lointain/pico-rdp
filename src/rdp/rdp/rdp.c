...
// under N64VIDEO_C (mirroring the fork, whose n64video.c includes n64video/rdp.c
// which includes rdp/*.c). The MIN/MAX/SIGN/RGBA macros, irand and clamp that
// the fork keeps in n64video.c are provided by n64video.c BEFORE this include.
//
...
// rdp_*, get_tmem, z_init_lut, ...). To avoid duplicate-symbol collisions, every
// such symbol is given INTERNAL LINKAGE here. In C++, a name whose FIRST
...
static struct rdp_state state[RDPX_PARALLEL_MAX_WORKERS];
...
// dither.c's noise helpers are external in the fork (shared with the parallel-rdp
// wrapper); here they collide with the oracle, so force internal linkage.
static void reseed_noise(uint32_t *seed, uint32_t x, uint32_t y, uint32_t offset);
