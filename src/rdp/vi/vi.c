...
static void vi_process_full_parallel(uint32_t worker_id) {
  ...
  struct Rgba color;
  struct Rgba nextcolor;
  struct Rgba scancolor;
  struct Rgba scannextcolor;

  vi_fetch_filter_func vi_fetch_filter_ptr =
      ctrl.type & 1 ? vi_fetch_filter32 : vi_fetch_filter16;

  uint32_t pixels = 0;
  uint32_t nextpixels = 0;
  uint32_t fetchbugstate = 0;

  int32_t xfrac = 0;
  int32_t yfrac = 0;
  int32_t line_x = 0;
  int32_t next_line_x = 0;
  int32_t prev_line_x = 0;
  int32_t far_line_x = 0;
  int32_t prev_scan_x = 0;
  int32_t scan_x = 0;
  int32_t next_scan_x = 0;
  int32_t far_scan_x = 0;
  int32_t prev_x = 0;
  int32_t cur_x = 0;
  int32_t next_x = 0;
  int32_t far_x = 0;
    ...
    for (x = 0; x < hres; x++, x_offs += x_add) {
      ...
      if (prev_line_x > cache_marker) {
        vi_fetch_filter_ptr(&viaa_cache[prev_line_x], frame_buffer, prev_x,
                            ctrl, vi_width_low, 0);
        vi_fetch_filter_ptr(&viaa_cache[line_x], frame_buffer, cur_x, ctrl,
                            vi_width_low, 0);
        vi_fetch_filter_ptr(&viaa_cache[next_line_x], frame_buffer, next_x,
                            ctrl, vi_width_low, 0);
      ...
      if (prev_line_x > cache_next_marker) {
        vi_fetch_filter_ptr(&viaa_cache_next[prev_line_x], frame_buffer,
                            prev_scan_x, ctrl, vi_width_low, fetchbugstate);
        vi_fetch_filter_ptr(&viaa_cache_next[line_x], frame_buffer, scan_x,
                            ctrl, vi_width_low, fetchbugstate);
        vi_fetch_filter_ptr(&viaa_cache_next[next_line_x], frame_buffer,
                            next_scan_x, ctrl, vi_width_low, fetchbugstate);
      ...
      if (ctrl.divot_enable) {
        if (far_line_x > cache_marker) {
          vi_fetch_filter_ptr(&viaa_cache[far_line_x], frame_buffer, far_x,
                              ctrl, vi_width_low, 0);
          cache_marker = far_line_x;
        }

        if (far_line_x > cache_next_marker) {
          vi_fetch_filter_ptr(&viaa_cache_next[far_line_x], frame_buffer,
                              far_scan_x, ctrl, vi_width_low, fetchbugstate);
          cache_next_marker = far_line_x;
        }

        if (line_x > divot_cache_marker) {
          divot_filter(&divot_cache[line_x], viaa_cache[line_x],
                       viaa_cache[prev_line_x], viaa_cache[next_line_x]);
          divot_filter(&divot_cache[next_line_x], viaa_cache[next_line_x],
        ...
        if (line_x > divot_cache_next_marker) {
          divot_filter(&divot_cache_next[line_x], viaa_cache_next[line_x],
                       viaa_cache_next[prev_line_x],
                       viaa_cache_next[next_line_x]);
          divot_filter(&divot_cache_next[next_line_x],
      ...
      bool const lerping = ctrl.aa_mode != VI_AA_REPLICATE && (xfrac || yfrac);
...
static bool vi_process_full(void) {
  bool const isblank = (ctrl.type & 2) == 0;
  ...
  bool const validh = hres > 0 && h_start < PRESCALE_WIDTH;
  int32_t const h_end =
      hres + h_start;  // note: the result appears to be different to VI_H_END
  int32_t const hrightblank = PRESCALE_WIDTH - h_end;
        ...
        case VI_MODE_COLOR:
          switch (ctrl.type) {
            case VI_TYPE_RGBA5551: {
              uint16_t const pix =
                  rdram_read_idx16((frame_buffer >> 1) + line + x);
            ...
            case VI_TYPE_RGBA8888: {
              uint32_t const pix =
                  rdram_read_idx32((frame_buffer >> 2) + line + x);
        ...
        case VI_MODE_DEPTH: {
          if (zb_address) {
            pixel->r = pixel->g = pixel->b =
                rdram_read_idx16((zb_address >> 1) + line + x) >> 8;
          }
          break;
        }
...
static bool vi_process_fast(void) {
  ...
  int32_t const filtered_width = maxhpass - minhpass;
  int32_t const filtered_height = (vres << 1) * V_SYNC_NTSC / v_sync;

  // re-calculate cropped 8 pixel area on the left and right from filtered mode
  int32_t const border_width = (hres - filtered_width) * hres_raw / hres;
...
static void vi_update_screen(void) {
  ...
  uint32_t const vi_control = *vi_reg_ptr[VI_STATUS];
  ...
  int32_t const vstartoffset = ispal ? 44 : 34;
