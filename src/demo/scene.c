...
// int16 model-space coordinate -> Q16.16 (value * 2^16).
static demo_fix scene_pos_to_fix(int16_t v) {
  // Multiply, not <<: left-shifting a negative value is UB. Value-identical.
  return (demo_fix)((int32_t)v * DEMO_FIX_ONE);
}
...
// int16 texel coordinate [0,64] -> texel-unit Q16.16 (value * 2^16).
