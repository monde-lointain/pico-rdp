...
exact failure mode `RDP_REQUIRE_ORTHODOXY` exists to catch.
...
> Note: the project's CI runs Orthodoxy only on the Linux job. macOS/Windows
> build with AppleClang/MSVC where the plugin is not engaged. You only need the
> plugin locally if you want enforcement on your own machine.
...
- If you install to a non-standard prefix, point the RDP configure at it with
  `-DCMAKE_PREFIX_PATH=/your/prefix`.

## 3. Configure RDP with enforcement live
...
If instead you see `Orthodoxy plugin not found` while `RDP_REQUIRE_ORTHODOXY=ON`,
configure aborts with a `FATAL_ERROR` — that is the enforcement-active assertion
doing its job. Common causes: clang major version mismatch, plugin not
installed, or wrong `CMAKE_PREFIX_PATH`.

## 4. Verify enforcement is actually live (the canary)

`RDP_REQUIRE_ORTHODOXY=ON` proves the plugin was *found*. The **canary** proves
it actually *rejects* bad code. `tests/orthodoxy_canary/canary.cc` contains
deliberate, unsuppressed violations (`auto`, `static_cast`, `class`). When the
plugin is active, that target is wired in and exposed as a `WILL_FAIL` ctest:

```sh
ctest --preset host -R rdp_orthodoxy_canary --output-on-failure
```
...
then configures with `-DRDP_REQUIRE_ORTHODOXY=ON` and runs `ctest` (which
includes the canary). Windows (MSVC) and macOS (AppleClang) build and test
without the plugin, covering portability. This keeps enforcement honest: a
regression that silences the plugin turns the Linux job red.
