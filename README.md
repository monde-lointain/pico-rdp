...
ctest --preset host
./build-host/src/picosystem_template        # run the demo

# Device (.uf2 for PicoSystem). Needs the arm-none-eabi toolchain + pico-sdk.
cmake --preset pico
cmake --build --preset pico
# -> build-pico/src/picosystem_template.uf2  (drag onto the PicoSystem in BOOTSEL mode)
```
...
pico-sdk is located via `PICO_SDK_PATH` (env or cache), defaulting to `~/development/repos/pico-sdk`, with a
`PICO_SDK_FETCH_FROM_GIT=ON` fallback. The pico build targets `PICO_BOARD=pimoroni_picosystem` and uses the
...
1. **Rename the project.** The CMake project, library namespace, executable, and window title all use the
   token `picosystem_template`. Replace it:
   - `CMakeLists.txt` — the two `project(picosystem_template ...)` lines
   - `src/*/CMakeLists.txt`, `src/CMakeLists.txt`, `tests/*/CMakeLists.txt` — the `picosystem_template::*`
