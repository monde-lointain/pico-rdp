...
// std::uniform_real_distribution is NOT, which is why portable_rng.hpp rolls its
// own integer-only [0,1) mapping. These tests pin:
...
TEST(PortableRng, Mt19937StreamIsStable)
{
	...
	uint32_t expected[5];
	for (int i = 0; i < 5; ++i)
		expected[i] = uint32_t(engine());

	std::mt19937 engine2{1337};
	for (int i = 0; i < 5; ++i)
		EXPECT_EQ(uint32_t(engine2()), expected[i]) << "mt19937 stream diverged at draw " << i;
}
