 ...
 * IMMEDIATELY AFTER init — with no clear, no randomize, no commands — the
 * hidden RDRAM buffers are BYTE-IDENTICAL.
 *
...
namespace RDP {
std::unique_ptr<ReplayerDriver> create_replayer_driver_angrylion(
    CommandInterface &player, ReplayerEventInterface &iface);
}
...
TEST(PostInitHiddenOracle, HiddenRdramSeedMatchesOracleByteForByte) {
  ...
  ASSERT_EQ(state.reference->get_hidden_rdram_size(),
            state.gpu->get_hidden_rdram_size());
  ...
  for (size_t i = 0; i < hidden_sz; ++i)
    ASSERT_EQ(hidden_ref[i], 3)
        << "oracle hidden byte " << i << " not seeded to 3";
